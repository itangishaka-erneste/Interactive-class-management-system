    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div className="pa-fade" style={{ background: '#fff', border: `1px solid ${t.border}`, borderRadius: 12, padding: 28, width: 380, maxWidth: '100%', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: 8, background: t.blue, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Globe size={18} color="#fff" /></div>
          <div>
...
function Header({ setSidebarOpen, title, onRefresh, refreshing, live }) {
  return (
...
export default function SuperAdminDashboard() {
  ...
  useEffect(() => {
    ...
    const socket = io(`${API_BASE}/superadmin`, { auth: { token: session.token } });
  ...
  if (!session?.token) {
    return (
      <div className="pa-root" style={{ minHeight: '100vh', color: t.text }}>
        <GlobalStyle />
        <LoginScreen onSuccess={handleLogin} notice={loginNotice} onBack={() => window.location.assign('/')} />
      </div>
    );
  }
    ...
    <div className="pa-root" style={{ minHeight: '100vh', color: t.text }}>
      ...
      <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <Header setSidebarOpen={setSidebarOpen} title={titles[section]} onRefresh={fetchSchools} refreshing={schoolsLoading} live={live} />
